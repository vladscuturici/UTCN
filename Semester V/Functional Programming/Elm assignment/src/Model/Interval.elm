...
compare (Interval intA) (Interval intB) =
    chainCompare (compareMaybeEnd intA.end intB.end)(Date.compare intA.start intB.start)

compareMaybeEnd : Maybe Date -> Maybe Date -> Order
compareMaybeEnd maybeEnd1 maybeEnd2 =
    case (maybeEnd1, maybeEnd2) of
        (Just _, Nothing) ->
            GT
        ...
        (Just end1, Just end2) ->
            Date.compare end1 end2

view : Interval -> Html msg
view (Interval { start, end }) =
    div [ class "interval" ] 
        [ div [ class "interval-start" ] [ Date.view start ]
        , div [ class "interval-end" ] 
            [ case end of
                Nothing -> text "Present"
                Just endDate -> Date.view endDate
            ]
        ]
